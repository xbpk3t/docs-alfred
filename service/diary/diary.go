...
type Task struct {
	...
	Item   []string `yaml:"item,omitempty" json:"item,omitempty"`     // 附加内容，类似tracking。用来标识该task的一些metrics之类的。比如说milestone类型的task（比如本月开销，购买了哪些东西，就可以写到item里）
	...
	X      bool     `yaml:"x,omitempty" json:"x,omitempty"` // 是否完成，默认true。如果false就是没完成
}
