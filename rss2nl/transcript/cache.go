...
// UpdateIndex updates the index.json file with current entries from metadata files.
func (c *Cache) UpdateIndex() error {
	entries, err := c.List()
	if err != nil {
		return err
	}

	idxPath := c.IndexFilePath()
	data, err := json.MarshalIndent(entries, "", "  ")
	if err != nil {
		return err
	}

	return os.WriteFile(idxPath, data, fileutil.FilePermPrivate)
}

// List returns all cached entries by scanning metadata files.
func (c *Cache) List() ([]CacheEntry, error) {
	...
	for _, entry := range entries {
		...
		metaPath := filepath.Join(c.baseDir, entry.Name(), "metadata.json")
		data, err := os.ReadFile(metaPath)
		if err != nil {
			continue
		}
