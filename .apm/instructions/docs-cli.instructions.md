...
- `docs-cli` 使用 Cobra，入口在 `docs-cli/cmd/root.go`。
...
- `docs-cli/cmd` 只做 command/flag/arg wiring 和用户输出。
- CLI 编排放在 `docs-cli/internal/usecase`。
- 可复用校验、解析、渲染逻辑放入已有 `pkg/data`、`pkg/gh`、`pkg/images`、`pkg/dotfiles`、`pkg/blog`、`pkg/docconfig`。
...
- 改 `docs-cli/cmd` 路由、flag、默认值时，补或运行 command/usecase 相关测试。
- 改 `pkg/data`、`pkg/parser`、`pkg/render` 时，跑对应 package 测试。
- 改 `pkg/gh` walker/find/append/check 时，跑 `go test ./pkg/gh ./docs-cli/internal/usecase/...`。
- 跨 workspace consistency 行为时，跑相关 `pkg/images`、`pkg/dotfiles`、`pkg/blog` 测试。
