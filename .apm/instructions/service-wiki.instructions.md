---
description: service/wiki classification, fetch, write, prompt, and dry-run rules
applyTo: "service/wiki/**"
---

# service/wiki Rules

- `service/wiki` 负责 fetch、classify、write 的核心逻辑；`wiki/cmd` 只做流程入口。
...
- 修改 `service/wiki/prompts/` 时，保持分类、摘要、失败落盘结构兼容。
