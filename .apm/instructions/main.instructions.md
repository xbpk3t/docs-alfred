...
不确定 CLI 子命令或 flag 时，先跑 `<cli> schema` 获取完整命令树、参数类型和默认值（JSON 输出）。不要猜用法，不要靠记忆。

agent 场景优先使用 JSON 输出：

- 全局生效：`export OUTPUT_FORMAT=json`
- 单次生效：`<command> --format json`
- flag 显式传入优先于环境变量

涉及副作用的命令（发邮件、写文件、调外部 API）必须先用 `--dry-run` 验证，确认无误后再执行真实操作。
