...
- `linear2nl` 使用 Cobra，入口在 `linear2nl/cmd/root.go`。
- 当前子命令：`morning`、`evening`。
- 保持 `--config/-c` 默认路径 `linear2nl/linear2nl.yml` 和 `--dry-run` 语义兼容。
...
- AI 配置走已有 `linear2nl/internal` 和 `pkg/ai` env fallback，不写死 key/base URL。
- 保持当前 CST/UTC+8 报告语义；不要改成本机隐式时区。
- 修改配置 schema 时保持向后兼容，除非任务明确要求 breaking change。

## 模板与 AI

- prompt 位于 `linear2nl/internal/prompts/`，template 位于 `linear2nl/cmd/templates/`。
...
- 改 config loader 跑 `go test ./linear2nl/internal`。
- 改 Linear client 跑 `go test ./linear2nl/linear`，测试用 fake server/mock。
