...
func TestRunWikiCheckMatchingStructure(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(ghRoot, "algo", "go.yml"), []byte("- topic: test\n  kind: type\n"), 0o600))
...
func TestRunWikiCheckTopicMismatch(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(ghRoot, "desktop", "GUI.yml"), []byte("- topic: terminal\n  kind: type\n"), 0o600))
